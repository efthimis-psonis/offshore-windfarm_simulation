...
import os
from typing import Callable, Dict, Any, Mapping, Iterable, Tuple, Optional, List
import xarray as xr
import pandas as pd
import numpy as np
...
def run_year_simulation(
    ...
    ds_year = xr.open_dataset(nc_path)
    ...
    if real_path and not os.path.exists(real_path) and verbose:
        print(f"⚠️ Real production not found for {year}; running without scoring.")
...
def run_simulation_for_years(
    ...
    for year in years:
        ...
        try:
            df_results, metrics_df = run_year_simulation(
                year=year,
                nc_path=nc_path,
                real_path=rp,
            ...
            results[year] = (df_results, metrics_df)
        except Exception as e:
            print(f"❌ Simulation failed for year {year}: {e}")
